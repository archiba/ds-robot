...
from pyatspi import STATE_DEFUNCT, Accessible
...
class JobActionBase:
    ...
    def check_app_alive(self, application: Accessible):
        logger.info('Checking the application availability.')
        states = application.getState().getStates()
        if STATE_DEFUNCT in states:
