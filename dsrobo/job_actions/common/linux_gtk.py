...
import pyatspi
from pyatspi import Accessible, Component
...
from dsrobo.util.linux_gtk import send_keys, press_key, type_write
...
class CommonSendHotkey(JobActionBase):
    ...
    def _impl(self,
              ...
              parameters: P) \
            -> tuple[int, Optional[Accessible]]:
        Component(element).grabFocus()
        registry = pyatspi.Registry()
        send_keys(registry, parameters.keys)
        time.sleep(parameters.wait_seconds)
        return True, element
...
class CommonSendControl(JobActionBase):
    ...
    def _impl(self,
        ...
        Component(element).grabFocus()
        registry = pyatspi.Registry()
        press_key(registry, parameters.key, parameters.press_duration)
        time.sleep(parameters.wait_seconds)
        return True, element
...
class CommonTypeWrite(JobActionBase):
    ...
    def _impl(self,
        ...
        Component(element).grabFocus()
        registry = pyatspi.Registry()
        type_write(registry, parameters.text)
        time.sleep(parameters.wait_seconds)
        return True, element
