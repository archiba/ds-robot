...
from dsrobo.util import ElementIdentifier
from dsrobo.util.linux_gtk import find_element_by_role_and_id, find_window_by_exact_name
...
class JobActionPipelineBase(object):
    ...
    def register_reserved_elements(self, application: Accessible):
        self.states.elements['application'] = application
        _, main_window_name = self.emulator_config.main_window_name.split('.', maxsplit=1)
        self.states.elements['main_window'] = find_window_by_exact_name(application,
                                                                        main_window_name)
        self.states.elements['menu_bar'] = find_element_by_role_and_id(self.states.elements['main_window'],
                                                                       ElementIdentifier(role="menu bar"))
        print('reserved elements:')
        print(self.states.elements)
    ...
    def __call__(self, application: Accessible):
